...
var blastRadius = Math.min(window.innerWidth, window.innerHeight)/5;  // size of the explosion for rockets, depending on window size
...
function launchFuse(type){
  ...
  fuseArray.push(new Fuse(type, canvas.width/2, canvas.height, speedX, speedY, size, color, pCount, lifeSpan, children));
}
...
function deleteParticles(){
  ...
  for(let i = 0; i < particlesArray.length; i++){
    if(particlesArray[i].x > 0 && particlesArray[i].y > 0 && particlesArray[i].x < canvas.width && particlesArray[i].y < canvas.height && particlesArray[i].size > 0.6 ){
...
window.addEventListener('keyup',
  function(event){
    // launch a fuse if spacebar is pressed
    if(event.keyCode == 32){
      launchFuse(Math.floor(Math.random()*10));
    }
    // activate or deactivate the bottom fountain
    else if(event.keyCode == 16){
      foutain = (foutain + 1)%2;
    }
    // if a number is pressed, launch the corresponding type of rocket
    else if(!isNaN(event.key)){
      launchFuse(parseInt(event.key));
    }
  }
);
